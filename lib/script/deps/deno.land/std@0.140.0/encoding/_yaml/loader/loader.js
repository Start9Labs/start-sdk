...
function generateError(state, message) {
    return new error_js_1.YAMLError(message, new mark_js_1.Mark(state.filename, state.input, state.position, state.line, state.position - state.lineStart));
}
...
function load(input, options) {
    ...
    throw new error_js_1.YAMLError("expected a single document in the stream, but found more");
}
exports.load = load;
